...
public class MyMap<K, V> {
    ...
    public int remove(K k) {
        ...
        keys.remove(result);
        data.remove(result);
        if (!(((this.keys == old_this_keys) && (this.data == old_this_data) && (size(this.keys) == size(old_this_keys_1) - 1) && (size(this.data) == size(old_this_data_1) - 1) && (result == 0 || result == 1 || result == 2) && (this.keys != null) && (this.data != null)))) {
            int mm = 2;
            assert (3 > mm);
        }
